...
const Penugasan = () => {
  ...
  const [assignments, setAssignments] = useState({ Pekerjaan: [], Pengabdian: [] });
  ...
  const handleAddClick = () => {
    setFormData({
      ...
      tanggal_mulai: '',
      tanggal_selesai: '',
      jenis: activeTab === 'pengabdian' ? 'Pengabdian' : 'Pekerjaan',
      file: null
    });
  ...
  const handleTabChange = (key) => {
    ...
    setFormData((prev) => ({
      ...prev,
      jenis: key === 'pengabdian' ? 'Pengabdian' : 'Pekerjaan'
    }));
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        setBanner({ message: `${data.deleted_count} data berhasil dihapus`, variant: 'success' });
  ...
  const processFile = (file, eventRef) => {
    ...
    if (file.type === 'application/pdf' && file.size <= 5 * 1024 * 1024) {
      ...
      setBanner({ message: 'File harus berformat PDF dan maksimal 5MB', variant: 'error' });
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.unit || !formData.penanggung_jawab || !formData.tanggal_mulai || !formData.file) {
      setBanner({ message: 'Mohon lengkapi field yang wajib diisi', variant: 'warning' });
      return;
    }
    ...
    try {
      ...
      const currentJenis = activeTab === 'pengabdian' ? 'Pengabdian' : 'Pekerjaan';
      
      apiFormData.append('file', formData.file);
      apiFormData.append('unit', formData.unit);
      apiFormData.append('ruang', formData.ruang || '');
      ...
      if (response.ok && data.success) {
        setBanner({ message: `${formData.jenis || 'Pekerjaan'} berhasil ditambahkan!`, variant: 'success' });
        ...
        setBanner({ message: data.message || 'Gagal menambahkan data', variant: 'error' });
      }
    } catch (error) {
      setBanner({ message: 'Terjadi kesalahan saat menambahkan data', variant: 'error' });
    } finally {
  ...
  const currentTabKey = activeTab === 'pekerjaan' ? 'Pekerjaan' : 'Pengabdian';
  ...
  return (
    <MainLayout>
      ...
      <header className={styles.pageHeader}>
        <h1 className={styles.pageTitle}>Pekerjaan & Pengabdian</h1>
        <p className={styles.pageSubtitle}>Riwayat pekerjaan, penempatan unit kerja, dan pengabdian</p>
      </header>
        ...
        <div className={styles.cardShell}>
          ...
          <div className={styles.headerRow}>
            <div>
              <h3 className={styles.sectionTitle}>
                {activeTab === 'pengabdian' ? 'Riwayat Pengabdian' : 'Riwayat Pekerjaan'}
              </h3>
            </div>
          ...
          {deleteMode && (
            <p className={styles.deleteNotice}>
              Pilih satu atau lebih kartu yang ingin dihapus, lalu klik Hapus.
            </p>
          )}
          ...
          {loading ? (
            ...
            <div className={styles.list}>
              <p style={{ textAlign: 'center', padding: '2rem' }}>
                Belum ada data {activeTab === 'pengabdian' ? 'pengabdian' : 'pekerjaan'}.
              </p>
            </div>
          ) : (
            <div className={styles.list}>
                ...
                <div className={styles.itemBody}>
                  ...
                  <div className={styles.itemGrid}>
                    <div className={styles.metaBlock}>
                      ...
                      <p className={styles.metaValue}>{formatDate(item.tanggal_mulai)}</p>
                    </div>
                    <div className={styles.metaBlock}>
                      <p className={styles.metaLabel}>Tanggal Selesai</p>
                      <p className={styles.metaValue}>{item.tanggal_selesai ? formatDate(item.tanggal_selesai) : 'Sekarang'}</p>
                    </div>
                    {item.ruang && (
                      <div className={styles.metaBlock}>
                        <p className={styles.metaLabel}>Ruang</p>
                        <p className={styles.metaValue}>{item.ruang}</p>
                      </div>
                    )}
                    <div className={styles.metaBlock}>
                    ...
                    <div className={styles.metaBlock}>
                      ...
                      <p className={styles.metaValue}>{item.jenis || 'Pekerjaan'}</p>
                    </div>
                    <div className={styles.metaBlock}>
      ...
      <Modal
        ...
        title={selectedItem?.unit || 'Detail Pekerjaan'}
        ...
        <div className={styles.viewDetail}>
          <div className={styles.detailGrid}>
            <div className={styles.detailRow}>
              ...
              <span className={styles.detailValue}>{formatDate(selectedItem?.tanggal_mulai)}</span>
            </div>
            <div className={styles.detailRow}>
              <span className={styles.detailLabel}>Tanggal Selesai</span>
              <span className={styles.detailValue}>{selectedItem?.tanggal_selesai ? formatDate(selectedItem?.tanggal_selesai) : 'Sekarang'}</span>
            </div>
            {selectedItem?.ruang && (
              <div className={styles.detailRow}>
                <span className={styles.detailLabel}>Ruang</span>
                <span className={styles.detailValue}>{selectedItem.ruang}</span>
              </div>
            )}
            <div className={styles.detailRow}>
      ...
      >
        <div className={styles.modalContent}>
          <p className={styles.metaValue}>
            Hapus {deleteTargets.length} data terpilih?
          </p>
          <p className={styles.metaLabel}>
            Tindakan ini akan menghapus data dari daftar riwayat.
          </p>
          {!!deleteTargets.length && (
      ...
      <Modal
        ...
        title={`Tambah ${activeTab === 'pengabdian' ? 'Pengabdian' : 'Pekerjaan'}`}
        size="medium"
        padding="normal"
      >
        <Form onSubmit={handleSubmit} className={styles.modalForm}>
          <Form.Row columns={2} className={styles.formRow}>
            ...
            {activeTab !== 'pengabdian' && (
              <Input
                label="Ruang"
                name="ruang"
                placeholder="Contoh: Ruang Multazam"
          ...
          <Form.Actions align="right" className={styles.modalActions}>
            ...
            >
              {isSubmitting ? 'Menyimpan...' : `Simpan ${activeTab === 'pengabdian' ? 'Pengabdian' : 'Pekerjaan'}`}
            </Button>
          </Form.Actions>
        </Form>
      </Modal>
