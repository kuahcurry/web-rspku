...
const Beranda = () => {
  ...
  const [penugasanData, setPenugasanData] = useState({ Pekerjaan: [], Pengabdian: [] });
  ...
  useEffect(() => {
    const fetchDistrictName = async () => {
      ...
      const isRegionCode = (v) => !!v && /^\d+(\.\d+)*$/.test(String(v));
      const capWords = (s = '') =>
        String(s).toLowerCase().replace(/\b\w/g, c => c.toUpperCase());

      if (!isRegionCode(user.district)) {
        // Already stored as a name — use it directly
        setDistrictName(capWords(user.district));
        return;
      }

      if (user.regency && isRegionCode(user.regency)) {
        const name = await getDistrictNameById(user.regency, user.district).catch(() => null);
  ...
  const fetchAllData = async () => {
    try {
      ...
      // Calculate Pekerjaan (2 types: Pekerjaan, Pengabdian)
      let penugasanCount = 0;
      if (penugasanData.success && penugasanData.data) {
        if (penugasanData.data.Pekerjaan?.length > 0) penugasanCount++;
      ...
      const storedPenugasan = penugasanData.success ? penugasanData.data : { Pekerjaan: [], Pengabdian: [] };
    ...
    >
      ...
      <div className={styles['page-body']}>
          ...
          <div className={styles['section-wrapper']}>
            ...
            <div className={styles['progress-grid']}>
              ...
              <Card variant='secondary' padding="normal">
                <div className={styles['progress-item']}>
                  <div className={styles['progress-header']}>
                    <span className={styles['progress-label']}>Pekerjaan</span>
        ...
        <section className={styles.section}>
          ...
          <div className={styles['education-section']}>
            <Card variant='secondary' padding="normal">
              <h4 className={styles['education-subtitle']}>Pekerjaan</h4>
              {penugasanData.Pekerjaan && penugasanData.Pekerjaan.length > 0 ? (
                penugasanData.Pekerjaan.slice(0, 1).map((item) => {
        ...
        <section className={styles.section}>
          <div className={styles['section-wrapper']}>
            <div className={styles['section-header']}>
              ...
              <Button variant="outline" size="small" icon={<FaChevronRight />} iconPosition="right" onClick={() => navigate('/etik-disiplin')}>
                Lihat Detail
              </Button>
            </div>
