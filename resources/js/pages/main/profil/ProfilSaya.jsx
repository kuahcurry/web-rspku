...
// Returns true for region identifiers: legacy numeric IDs and wilayah.id dotted codes
const isRegionId = (value) => !!value && /^\d+(\.\d+)*$/.test(String(value));

// Capitalise each word (mirrors the API's mapOption format)
const capWords = (text = '') =>
  String(text).toLowerCase().replace(/\b\w/g, (c) => c.toUpperCase());
...
const ProfileSaya = () => {
  ...
  useEffect(() => {
    ...
    if (user) {
      setFormData({
        ...
        tanggalLahir: toDateInput(user.tanggal_lahir),
  ...
  // Fetch region names — if value is already a plain name (non-numeric), use it
  // directly; only call the external API when value is a numeric ID.
  const fetchRegionNames = async (userData) => {
    if (!userData) return;

    try {
      const resolve = async (value, apiFn) => {
        if (!value) return '';
        if (!isRegionId(value)) return capWords(value); // already a name
        const result = await apiFn();
        return result || '';
      };

      const [provinceName, regencyName, districtName, villageName] = await Promise.all([
        resolve(userData.province, () => getProvinceNameById(userData.province)),
        resolve(userData.regency,  () => getRegencyNameById(userData.province, userData.regency)),
        resolve(userData.district, () => getDistrictNameById(userData.regency, userData.district)),
        resolve(userData.village,  () => getVillageNameById(userData.district, userData.village)),
      ]);
